...
We decided to:
1. **Two-Tier Heuristic Hardware Detection**: Probe hidden WebGL canvas attributes (`UNMASKED_RENDERER_WEBGL`, `UNMASKED_VENDOR_WEBGL`) to identify software rasterizer signatures (`llvmpipe`, `softpipe`, `swiftshader`, `mesa off-screen`, `vmware`, `virtualbox`, `qemu`, `virgl`). Combine with `navigator.hardwareConcurrency <= 2` and `prefers-reduced-motion` accessibility settings.
...
3. **Hydration-Safe & Anti-Flash PerformanceProvider**: Manage initial state safely without triggering React 19 hydration mismatches. Synchronize the `.lite-mode` class directly to `document.documentElement` during client-side mount effects.
4. **Opacity Bleed-Through Defense**: Stripping `backdrop-filter` from semi-transparent elements (`bg-surface-1/80`, `bg-surface-1/90`) causes scrolled background text to bleed through navigation headers, sidebars, modals, and popovers. The `.lite-mode` CSS layer explicitly forces solid, 100% opaque background colors (`var(--surface-1) !important`) on all surface containers (`header`, `aside`, `[role="dialog"]`, `[data-floating-panel]`, `.surface-panel`), guaranteeing crisp contrast.
...
6. **Tri-State User Override Control**: Provide an accessible three-state toggle (`auto` -> `lite` -> `full`) persisted in `localStorage` (`flowstate-performance-mode`) via `<PerformanceToggle />` in the top header, allowing users to override automatic heuristics at any time.
