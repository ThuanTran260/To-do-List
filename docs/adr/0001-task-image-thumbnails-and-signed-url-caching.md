...
7. **Batch Signed URLs, Viewport Gating & Re-sign on Expiry**: Implement `useBatchSignedUrls(paths)` using Supabase `createSignedUrls(paths, 604800)` (7 days). Gated by viewport visibility (only request signed URLs for items entering or near the viewport). Handle both result branches: per-item error mapping and top-level batch network/auth errors (triggering query retry and temporary placeholder rendering). Differentiate per-item errors: missing object (404) renders placeholder without retry storm; expired signature or auth invalidation (403) triggers automatic re-sign for that path. Update `docs/security.md:67` to document the 7-day TTL policy.
